...
train = pd.read_csv(os.path.join(input_fd,'train.csv'))
test = pd.read_csv(os.path.join(input_fd,'test.csv'))
...
'''
# Duplicated Row
train_cols=train.drop(["ID","y"], axis=1).copy()

cat_cols = []

for c in train_cols.columns:
    cat_cols.append(c)
             
train['is_duplicated'] = train.duplicated(cat_cols, keep=False)
train['y_mean'] = train.copy().groupby(cat_cols)['y'].transform('mean')
train['y_max'] = train.copy().groupby(cat_cols)['y'].transform('max')
train['y_min'] = train.copy().groupby(cat_cols)['y'].transform('min')
train['y_median'] = train.copy().groupby(cat_cols)['y'].transform('median')
train=train.drop(["is_duplicated"], axis=1).copy()

test = pd.merge(test,train.drop(["ID","y"], axis=1).copy(),on=cat_cols,how='left')
test = test.drop_duplicates()
test['y_mean'].fillna(test['y_mean'].dropna().mean(), inplace=True)
test['y_max'].fillna(test['y_max'].dropna().max(), inplace=True)
test['y_min'].fillna(test['y_min'].dropna().min(), inplace=True)
test['y_median'].fillna(test['y_median'].dropna().median(), inplace=True)
'''
...
for col in cat_cols:
    for func in ['mean', 'min', 'max', 'median', 'std']:
        train[func + '_' + col] = train[[col, 'y']].groupby(col).transform(func)
        test[func + '_' + col] = test[col].map(train[[col, 'y']].groupby(col).agg(func).iloc[:,0])
        test[func + '_' + col].fillna(test[func + '_' + col].dropna().agg(func), inplace=True)
'''
...
def model():
    ...
    model.compile(loss='mean_squared_error', # one may use 'mean_absolute_error' as alternative
                  optimizer='rmsprop',
                  metrics=[r2_keras,"mae"] # you can add several if needed
                 )
