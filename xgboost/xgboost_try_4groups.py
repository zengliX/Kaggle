...
# group indicators 
newG_cols = list(filter(lambda x: 'new_group_' in x, TRAIN.columns))
train_g = TRAIN.loc[:,newG_cols]
test_g = TEST.loc[:,newG_cols]

TRAIN.drop(newG_cols,axis=1,inplace=True)
TEST.drop(newG_cols,axis=1,inplace=True)
...
#params_list = param_gen(eta=[0.1,.05],max_depth=[2],sub=[1],col_sub=[1])
...
def myCV(xgb_params,g):
    # select index
    g_ind_train = train_g.index[train_g[g]==1]
    mytrain = TRAIN.loc[g_ind_train,:]    
...
for xgb_params in params_list:
    ...
    for g in newG_cols[:4]:
        print('-----------')
        print('working on group:',g,'\n')
        print("working on parameters:",xgb_params,'\n')
        cur_cv = myCV(xgb_params,g)
        cv_out[g].append({'pars':xgb_params,'fit':cur_cv})
        
    
        # report
        print('group',g,"current cv results:",cur_cv)
        print('------------')
    ct+=1
...
final_pars = {}
for g in newG_cols[:4]:
    temp = [x['fit']['test_r2_mean'] for x in cv_out[g]]
    print('mean test_r2 for group',g,':',temp)
    loc = np.argmax(temp)
    final_pars[g] = cv_out[g][loc]['pars']
