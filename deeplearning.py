...
def model():
    model = Sequential()
    #input layer
    model.add(Dense(input_dims, input_dim=input_dims, kernel_initializer='normal')) #, kernel_constraint=maxnorm(5)
    model.add(BatchNormalization())
    model.add(Activation('relu'))
    model.add(Dropout(0.2)) # Reduce Overfitting With Dropout Regularization
    
    # hidden layers
    model.add(Dense(input_dims, kernel_initializer='normal'))
    model.add(BatchNormalization())
    model.add(Activation(act_func))
    model.add(Dropout(0.2))
    
    model.add(Dense(input_dims//4, kernel_initializer='normal'))
    model.add(BatchNormalization())
    model.add(Activation(act_func))
    model.add(Dropout(0.2))
  
    model.add(Dense(input_dims//2, kernel_initializer='normal'))
    model.add(BatchNormalization())
    model.add(Activation(act_func))
    model.add(Dropout(0.2))
